...
| Виньетка (`vignette`) | лёгкая | Затемнение углов — «фильтр», сдвигает много пикселей |
...
| Геометрия (зум 5–8 % + **микро-поворот ±1°** + кроп + обрезка кадров) | **вкл** | Главный рычаг против дедупа; может подрезать текст у самых краёв |

Фильтры (colorbalance/vignette/eq/hue) и аудио-сдвиг тона применяются **всегда** —
они не режут кадр. Геометрия по умолчанию **включена** (жёстко). В боте режим
выбирается **прямо в диалоге** после числа версий: 🛡 **Безопасно** (без геометрии —
для видео с текстом у самых краёв) или ⚡️ **Жёстко** (с геометрией). Переменная
