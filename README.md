...
Режим геометрии пользователь выбирает **прямо в боте** после выбора числа версий:
🛡 **Безопасно** (без геометрии — по умолчанию) или ⚡️ **Агрессивно**
(зум+кроп+обрезка). Переменная `UNIQ_GEOMETRY` задаёт лишь значение по умолчанию
для прямого вызова ядра из кода (см. ниже) — на диалог бота она не влияет.
